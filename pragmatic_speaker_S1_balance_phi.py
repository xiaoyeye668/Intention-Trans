...
def utility_for_S1(values, costs, L, phi, lib='np'):
    if lib == 'at':
        lib = at
    else:
        lib = np
    ...
    # Shape (utterance)
    exp_values = (values*L0).mean(
        axis=1
    )

    utility = (
        # shape (goal condition, utterance, state)
        # dims: (utility component, goal condition, 1)
        # informational component (value of phi, utterance, state)
        phi[:,None,None] * lib.log(L0)
        # social component (value of phi, utterance, 1)  
        + (1-phi)[:,None,None] * exp_values[:,None]
        # (utterance, 1)
        - costs[:,None]
    )
    print('<<<<<< phi ',phi.eval().shape, phi.eval())   
    return utility    

def S1_likelihood(alpha, values, costs, L, phi, lib='np'):
    # shape (goal condition, utterance, state)
    utilities = utility_for_S1(values, costs, L, phi) #shape:(3,10,5)
...
def factory_yoon_model(dt, dt_meaning):    
    ...
    with pm.Model() as yoon_model:
        ...
        #values = beta * np.array([1, 2, 3, 4, 5])
...
if __name__=='__main__':
    ...
    az.to_netcdf(
        yoon_trace, 
        'traces/S1_1wTune2w_LS2map_phi_newalpha.cdf'
    )
    az.to_netcdf(ppc, 'ppcs/ppc_S1_1wTune2w_LS2map_phi_newalpha.cdf')
